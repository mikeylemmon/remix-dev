...
const flattenRemixImportDeclarations = remixImportDeclarations => {
  ...
  remixImportDeclarations.forEach(declaration => {
    let {
      source,
      specifiers
    } = declaration.node; // Side-effect imports like `import "remix"` not produced flattened specifiers
  ...
  return {
    visitor: {
      Program(program) {
        ...
        let currentRemixImportDeclarations = findRemixImportDeclarations(program); // check for magic `from "remix"` imports

        let magicRemixImports = currentRemixImportDeclarations.filter(path => path.node.source.value === "remix");
        if (magicRemixImports.length === 0) return; // flatten current Remix import declarations to specifiers

        let currentRemixImports = flattenRemixImportDeclarations(currentRemixImportDeclarations); // convert current remix imports to new imports

        let newRemixImports = convertToNewRemixImports(currentRemixImports, options); // group new imports by source

        let newRemixImportsBySource = Array.from(groupImportsBySource(newRemixImports)); // create new import declarations
        ...
        currentRemixImportDeclarations[0].insertAfter(newRemixImportDeclarations); // remove old remix import declarations
