...
const codemod = async (projectDir, options) => {
  ...
  await task.task(`Removing magic ${code("remix")} package from dependencies`, async () => {
    ...
    let otherDevDeps = devDeps.filter(({
      name
    }) => !remix.isRemixPackage(name)); // detect `@remix-run/serve`

    let remixServeInstalled = remixDeps.map(({
      name
    }) => name).includes("@remix-run/serve"); // determine latest Remix version that is compatible with project

    let remixVersionSpec = getRemixVersionSpec([...remixDeps, ...remixDevDeps]);
    pkg.update({
      dependencies: { ...dependencies.unparse(otherDeps),
      ...
      devDependencies: { ...dependencies.unparse(otherDevDeps),
  ...
  await task.task(replaceRemixMagicImportsText(), async spinner => {
    ...
    let changes = false; // run the transform on each of those files
