...
const autoDetectPostinstallRuntime = packageJson => {
  ...
  if (postinstall$1 === undefined) return undefined; // match `remix setup <runtime>` in `postinstall` script
...
const detectRuntime = (packageJson, adapter) => {
  ...
  if (postinstallRuntime) return postinstallRuntime; // infer runtime from adapter

  if (adapter) return adapterToRuntime[adapter]; // @remix-run/serve uses node
